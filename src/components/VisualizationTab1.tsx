...
// Komponen SonicWaves
const SonicWaves = ({ position = [0, 0, 0], active = true, distance = 10 }) => {
  const [waves, setWaves] = useState([]);
  const maxWaves = 3;
  const waveSpeed = 0.1;
  const waveInterval = 0.8;
  const maxScale = distance * 0.1; // Skala gelombang disesuaikan dengan jarak
...
// Komponen RobotModel dengan SonicWaves
const RobotModel = ({ pwm = 0, position = [0, 0, 0], distance }: { pwm: number; position: THREE.Vector3 | [number, number, number]; distance: number }) => {
  ...
  // Ambil posisi z robot
  const robotZ = Array.isArray(position) ? position[2] : position.z;
  // Jarak ke dinding (dinding di z = 7.5)
  ...
  return (
    <group ref={bodyRef} position={position}>
      ...
      {/* Garis jarak */}
      <mesh position={[0, 0.2, 1 + lineLength / 2]}>
        <boxGeometry args={[0.02, 0.02, lineLength]} />
        <meshStandardMaterial color="#fff" />
      </mesh>

      <Text position={[0.5, -0.3, 1 + lineLength / 2]} color="#fff" fontSize={0.3} anchorX="left">
        {distance.toFixed(1)}cm
      </Text>

      {/* Efek sonic waves di posisi sensor */}
      <SonicWaves position={[0, 0.3, 1.0]} active={true} distance={distance} />
    </group>
  );
};

// Komponen Environment (tetap sama)
...
// Komponen PWMVisualizer (tetap sama)
...
// Komponen VisualizationTab (tetap sama)
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setRobotPosition(prev => {
        ...
        newPosition.z = Math.max(0, Math.min(6.5, newPosition.z));
        return newPosition;
      });
    }, 100);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="h-[500px] bg-black/30 backdrop-blur-sm rounded-lg overflow-hidden relative">
          <Canvas>
            ...
            <RobotModel pwm={pwm} position={robotPosition} distance={distance} />
